...
const AddPlacesForm = () => {
        ...
        if (id) {
            axios.get("/places/" + id)
                .then(response => {
                    ...
                    updateStateString("user", userData.user);
    ...
    const savePlace = async (event) => {
        ...
        if (!id) {
            // Adding New Place
            await axios.post("api/addNewPlace", placeData);
        }
        else {
            // Editing Existing Place
            await axios.put("api/editPlace", { id, ...placeData });
        }
    ...
    return (
        <>
            {/* <AccountNav/> */}
            <form className="w-4/5 mx-auto" onSubmit={savePlace} encType="multipart/form-data">
                    ...
                    {state.addedPhotos.length > 0 && state.addedPhotos.map((link, index) => {
                        return (
                            <div className="h-32 flex relative" key={index} >
                                <img className="rounded-2xl w-full object-cover position-center" src={`${link}`} alt="" />
